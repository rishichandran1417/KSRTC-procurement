...
import {
  ...
  Truck, BarChart3, Settings as SettingsIcon, X,
} from "lucide-react";
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex h-full w-64 flex-col border-r border-[--color-border] bg-[--color-surface-0] transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 ${
      ...
      >
        <div className="flex items-center justify-between border-b border-[--color-border] px-5 py-4">
          ...
          <button
            onClick={closeMobile}
            className="rounded p-1 text-[--color-ink-500] hover:bg-[--color-surface-1] lg:hidden"
